...
class TestQuatMultiply:
    ...
    def test_double_rotation(self):
        # Two 90-degree rotations around Z should equal 180 around Z
...
class TestQuatConjugate:
    ...
    def test_inverse_property(self):
        # q * conj(q) should equal identity for any unit quaternion
...
class TestRandomQuaternion:
    ...
    def test_distribution_coverage(self):
        ...
        # w component of uniform SO(3) quaternions should span most of [-1, 1]
        assert min(ws) < -0.8
        assert max(ws) > 0.8
...
class TestRandomQuaternionWithinAngle:
    ...
    def test_full_so3_fallback(self):
        # When max_angle >= 2*pi, it should fall back to full SO(3) sampling.
        # We just verify it produces valid unit quaternions with broad coverage.
        ...
        for _ in range(200):
            ...
            _, angle = quat_to_axis_angle(q)
            angles.append(angle)
        # Should have some large rotations near pi
...
class TestAxisAngleRoundtrip:
    ...
    def test_small_angle(self):
        # Very small angle should be numerically stable
