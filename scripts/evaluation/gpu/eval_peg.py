...
def evaluate(
    ...
    print(f"\nPeg MJX eval (clearance={clearance * 1000:.1f}mm, n={n_episodes})")
    print(
        f"  success={success_rate:.1f}%  drops={drop_rate:.1f}%  force={mean_force:.2f}N  ins_time={mean_ins_time:.1f}steps"
    )

    if record_video and video_frames:
        vdir = Path(video_dir)
        vdir.mkdir(parents=True, exist_ok=True)
        video_path = vdir / f"peg_mjx_evaluation_{clearance * 1000:.0f}mm.mp4"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate SBX-trained peg-in-hole model")
    parser.add_argument("--model-path", required=True, help="Path to saved SBX model .zip")
