...
from dexterous_hand.config import MjxTactileTrainConfig
from dexterous_hand.curriculum.callbacks import (
    AssemblyCurriculumCallback,
    scale_stage_starts,
)
from dexterous_hand.envs.gpu.peg_tactile_env import ShadowHandPegTactileMjxEnv
from scripts.training._common import RewardInfoLoggerCallback, setup_sb3_logger

def train(config: MjxTactileTrainConfig) -> None:

    run_dir = Path("runs") / f"tactile_mjx_{config.num_envs}env_{config.seed}"
    ...
    wandb.init(
        ...
        name=f"tactile-mjx-{config.num_envs}env",
        config=wandb_config,
    )

    vec_env = ShadowHandPegTactileMjxEnv.from_config(config)
...
def parse_args() -> MjxTactileTrainConfig:
    parser = argparse.ArgumentParser(description="Train Shadow Hand tactile peg (MJX + SBX SAC)")
