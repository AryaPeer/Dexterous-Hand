...
class GraspRewardCalculator:
    def __init__(self, config: RewardConfig, table_height: float) -> None:
        """Set up the grasp reward calculator.

        Args:
            config: reward weights and thresholds
            table_height: table surface height (needed for lift calculations)
        """
    ...
    def compute(
        ...
        """Compute the total grasp reward from current state.

        Combines reaching, grasping, lifting, holding rewards plus penalties
        for dropping the object and jerky actions.

        Args:
            fingertip_positions: (5, 3) world positions of each fingertip
            object_position: (3,) object center
            object_linear_velocity: (3,) how fast the object is moving
            num_fingers_in_contact: how many fingers are touching it
            actions: (20,) current actions
            previous_actions: (20,) last step's actions (for smoothness penalty)

        Returns:
            total: weighted reward sum
            info: breakdown of each reward component + metrics
        """
        info: dict[str, float] = {}
