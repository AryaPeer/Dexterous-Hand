...
@dataclass
class NameMap:
    # All the MuJoCo IDs we need, resolved once at startup so we don't look them up every step
...
def build_scene(
    ...
    """Build the grasping scene: hand pointing down, table with an object on it.

    Args:
        config: scene settings, uses defaults if None

    Returns:
        model, data, name_map with all the IDs we need
    """
    if config is None:
        config = SceneConfig()
...
def _resolve_names(model: mujoco.MjModel, spec: mujoco.MjSpec) -> NameMap:
    """Look up all the MuJoCo IDs we need and pack them into a NameMap.

    Args:
        model: compiled MuJoCo model
        spec: the spec we built the model from
    """
...
def get_object_half_height(geom_type: int, geom_size: list[float]) -> float:
    """Figure out how tall a geom is so we can place it on the table correctly.

    Args:
        geom_type: MuJoCo geom type (box, sphere, cylinder, etc.)
        geom_size: size params [x, y, z]

    Returns:
        Half-height for placing the object right above a surface.
    """
