...
@dataclass
class PegNameMap:
    """All the MuJoCo IDs for the peg-in-hole scene, resolved at startup."""
...
def build_peg_scene(
    ...
    """Build the peg-in-hole scene: hand, table, peg, and a hole with walls.

    Args:
        config: peg scene settings, uses defaults if None
...
def _resolve_peg_names(model: mujoco.MjModel, config: PegSceneConfig) -> PegNameMap:
    """Look up all the MuJoCo IDs for the peg scene."""
