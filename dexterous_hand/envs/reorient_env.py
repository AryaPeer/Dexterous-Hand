...
class ShadowHandReorientEnv(gym.Env):
    ...
    def __init__(
        ...
        n_obs = 115  # 24+24+3+4+3+3+4+4+15+5+6+20
    ...
    def reset(
        ...
        *,
        ...
        """Reset — cube back in palm, new target orientation.

        @param seed: random seed
        @type seed: int | None
        @param options: unused
        @type options: dict[str, Any] | None
        @return: (obs (115,), info with targets_reached count)
        ...
        # randomize hand joints slightly
        ...
        # get palm position for cube placement
        mujoco.mj_forward(self.model, self.data)
    ...
    def set_curriculum_stage(self, max_angle: float) -> None:
        """Set max target rotation angle (called by curriculum callback)."""
